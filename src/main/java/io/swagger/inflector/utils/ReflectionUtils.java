...
/*
    public Class<?> getParameterSignature(Parameter parameter, Map<String, Model> definitions) {
        if (parameter instanceof SerializableParameter) {
            ...
            Class<?> output = getClassFromProperty(type, format);
            if(!Null.class.equals(output)) {
                return output;
            }
            ...
            if (model instanceof RefModel) {
                ...
                return detectModel(ref.getSimpleRef(), referencedModel, null);
            } else if (model instanceof ArrayModel) {
                ArrayModel am = (ArrayModel) model;
                Property referencedProperty = am.getItems();
                if(referencedProperty instanceof RefProperty) {
                    RefProperty ref = (RefProperty) referencedProperty;
                    Model innerModel = definitions.get(ref.getSimpleRef());
                    Class<?> inner = detectModel(ref.getSimpleRef(), innerModel, "array");
                    
                    return inner;
                }
                else {
                    Class<?> inner = getClassFromProperty(referencedProperty.getType(), referencedProperty.getFormat());
                    if(!Null.class.equals(inner)) {
                        String fqName = "[L" + inner.getCanonicalName() + ";";
                        try {
                          return Class.forName(fqName);
                        } catch (ClassNotFoundException e) {
                          LOGGER.error("unable to instantiate array " + fqName);
                        }
                    }
                }
                return List.class;
    ...
    public Class<?> detectModel(String name, Model model, String container) {
        ...
        try {
            ...
            return ReflectionUtils.class.getClassLoader().loadClass(fqName);
        } catch (ClassNotFoundException e) {
            // continue
        }
        // try with config prefix
        if (config.getModelPackage() != null && name.indexOf(".") == -1) {
            String fqName = config.getModelPackage() + "." + name;
            try {
              if("array".equals(container)) {
                fqName = "[L" + fqName + ";";
              }
