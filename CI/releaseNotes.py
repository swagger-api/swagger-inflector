...
def allPulls(releaseDate):
    ...
    content = ghApiClient.readUrl('repos/swagger-api/swagger-inflector/pulls?state=closed&base=master&per_page=100')
    for l in content:
...
def lastReleaseDate(tag):
    content = ghApiClient.readUrl('repos/swagger-api/swagger-inflector/releases/tags/' + tag)
...
def addRelease(release_title, tag, content):
    ...
    content = ghApiClient.postUrl('repos/swagger-api/swagger-inflector/releases', payload)
    return content
